...
def polyder(coefficients: xr.DataArray, poly_type: str) -> xr.DataArray:
    ...
    if poly_type == 'polynomial':
        ...
        v = [np.polynomial.hermite.Hermite(coefficients.isel(axis=i).values).deriv(m=1).coef for i in range(3)]
...
class OrbitPolyfitIterpolator:
    ...
    def from_position(
        ...
        if poly_type == 'polynomial':
            ...
            polyfit_results = xr.Dataset({'axis': [0, 1, 2], 'degree': np.arange(deg, -1, -1)})
    ...
    def position(
        ...
        if self.poly_type == 'polynomial':
            ...
            position = position.assign_coords({time.name: time}).rename('position')
    ...
    def velocity(
        ...
        velocity_coefficients = polyder(self.coefficients, self.poly_type) * S_TO_NS

        velocity: xr.DataArray
        if self.poly_type == 'polynomial':
            velocity = xr.polyval(time - self.epoch, velocity_coefficients)
            velocity = velocity.assign_coords({time.name: time}).rename('velocity')
        elif self.poly_type == 'hermite':
            v = [np.polynomial.hermite.hermval(((time - self.epoch)/10**9).astype('float64'), 
